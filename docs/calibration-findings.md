...
### The detector catches fewer than half

At a 5.3 % false-positive rate on human prose, Nonio flags **47 % of
machine-generated text** — 95 % CI [36.1 %, 58.3 %], n=72.

That is a modest result and it should be read as such. Slightly better than a
coin flip on the generated side, while wrongly flagging one human text in twenty.
An earlier run with n=18 gave 66.7 %; that figure was optimistic noise, and the
same correction that halved the bias estimate also cut the sensitivity claim.

Both signals are zero-shot detectors running on a 0.5B model against text
generated by a 2B model from a different family. Being hard is the expected
outcome; publishing a flattering number from a small sample would not have made
it easier.

The honest summary of Nonio's current state: **it does not detect
machine-generated Spanish well enough to be useful for anything consequential**,
and it says so rather than inflating the number.

### The first bias measurement was noise
...
## Sensitivity, separately from bias

The two things fail independently and should not be conflated:

- **Bias** (FR-030) is about whether the instrument is *unfair*. Verdict:
  inconclusive.
- **Sensitivity** (TPR) is about whether it is *useful*. Measured: 47 %.

A perfectly fair instrument that catches half of what it looks for is fair and
not very useful. Nonio currently is both, and neither figure excuses the other.
